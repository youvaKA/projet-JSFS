...
*/
const registerForm = (_,res) => res.redirect('pages/register.html');
/*
...
const loginForm = (_,res) => {
  console.log('-------loginForm est appelée-------');
...
const login = async (req, res) => {
    try {
      ...
      if (user) {
        ...
        res.cookie('token', token,  { maxAge : 3600000, httpOnly: true, sameSite : 'strict' })  // secure : true (avec https)
...
const logout = (req,res) => {
  res.cookie('token', '',  { maxAge : 600, httpOnly: true, sameSite : 'strict' }) // secure : true
